...
# CSS class suffix → category (used when title element is missing)
CLASS_TO_CATEGORY = {
...
class LinkedInScraper:
    ...
    # ══════════════════════════════════════════════════════════════════
    # PEOPLE — reads the ACTIVE carousel slide only, using direct
    # Playwright scroll-into-view + page.evaluate on visible text nodes.
    # This bypasses the Ember lazy-render problem where off-screen cards
    # have empty text nodes until they enter the viewport.
    # ══════════════════════════════════════════════════════════════════

    def _get_active_slide_index(self) -> int:
        """Returns 0-based index of the currently active carousel dot."""
        try:
    ...
    def _scroll_card_into_view(self, card_locator):
        """Scrolls a card element into the viewport so Ember renders its text."""
        try:
    ...
    def _read_active_cards(self) -> dict:
        """
        Reads all cards visible in the currently active carousel slide.

        Strategy:
        1. Find the active <li> carousel item (data-item-index matches active dot).
        2. Scroll each card into view to trigger Ember text rendering.
        3. Read category title via Playwright locator (now rendered).
        4. Read each button text via page.evaluate on the element handle
           (bypasses Playwright visibility filter, reads raw DOM text).
        5. Fall back to full innerText line-parsing if buttons are empty.
        """
        collected = {}

        # Find active carousel item index
        active_idx = self._get_active_slide_index()

        # Locate the active carousel list item
        # LinkedIn uses data-item-index on <li> elements
        active_item = None
        try:
            items = self.page.locator("li.artdeco-carousel__item").all()
            if items:
                if active_idx < len(items):
                    active_item = items[active_idx]
        except:
            pass

        # If carousel item targeting fails, fall back to all visible cards
        if active_item:
            card_parent = active_item
        else:
            card_parent = self.page

        # Get all bar-graph cards within the active slide
        card_selectors = [
        ...
        # Also try generic insight-container if specific ones found nothing
        if not found_cards:
            try:
                for card in card_parent.locator("div.insight-container").all():
                    found_cards.append(card)
            except:
                pass

        for card in found_cards:
            # Scroll into view to force Ember to render text nodes
            self._scroll_card_into_view(card)

            # --- Get category ---
            category = None

            # Method 1: insight-container__title
            try:
            ...
            # Method A: page.evaluate on each button element handle
            # This reads raw DOM text regardless of Playwright visibility rules
            try:
            ...
            # Method B: full card innerText line-parse fallback
            if not entries:
                try:
    ...
    def scrape_people(self, slug, name):
        ...
        # Scroll down to make the people section visible
